	        ...
	        if(rows.length == 0){
				...
				connection.query(sql,insert_params, (err, rows, fields) => {
			        if (err){
						...
						res.status(200).json({});
						//create payload
						const payload = JSON.stringify({title:"Notification engine",options : {    body: 'Thank you for subscribing...',    icon: 'http://localhost/notification_engine/images/icon.png',    badge: '../images/badge.png'  }});
	        	...
	        	res.status(200).json({});
				//create payload
				const payload = JSON.stringify({title:"Notification engine",options : {    body: 'You are already subscribed with us...',    icon: 'http://localhost/notification_engine/images/icon.png',    badge: 'images/badge.png'  }});
				//pass object into sendNotification
				webpush.sendNotification(subscription,payload)
				.then(function(response){
					console.log('api response--------'+JSON.stringify(response));
